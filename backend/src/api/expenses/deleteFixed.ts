...
/**
 * @api {DELETE} /api/projects/:projectId/fixed-expenses/:yearMonth 固定費月次削除
 * @description
 *   - 指定年月の固定費データを全件削除し、FixedExpenseMonth レコードも削除する
 *   - 削除後は前月の固定費が継承される
 *   - editor 以上の権限が必要
 *
 * @request
 ...
 *
 * @response
 *   成功時: { success: true, code: '', message: 'Fixed expenses deleted successfully',
 *             data: { yearMonth } }
 *   失敗時:
 ...
 *     "success": true,
 *     "code": "",
 *     "message": "Fixed expenses deleted successfully",
...
router.delete('/:yearMonth', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  // 該当プロジェクト・年月の固定費を全件削除
  await FixedExpense.destroy({ where: { project_id: projectId, year_month: yearMonth } });

  // 保存済み年月レコードを削除（以降の継承ロジックが再度適用されるようにする）
  await FixedExpenseMonth.destroy({ where: { project_id: projectId, year_month: yearMonth } });
